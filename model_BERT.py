...
# @File : predict.py
# @Project: Douban_Bert
# @CreateTime : 2024/5/20 上午12:08:22
# @Version：V 0.1
'''
模型训练和评估
'''
...
def train(batch_size,EPOCHS):
    ...
    model = BertForSequenceClassification.from_pretrained("hfl/chinese-bert-wwm", num_labels=2)#使用chinese-bert-wwm预训练模型
    ...
    train = read_data('/home/u2021213473/jupyterlab/Email_filter/Bert/data/filtered_train.csv')
    val = read_data('/home/u2021213473/jupyterlab/Email_filter/Bert/data/filtered_test.csv')
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            acc = corrects.float().mean()  # 转换为float，并计算平均值
            batch_accuracy.append(acc.item())
        avg_train_loss = total_train_loss / len(train_dataloader)
        avg_train_acc = sum(batch_accuracy) / len(batch_accuracy)
        train_time = format_time(time.time() - t0)

        log.info('====Epoch:[{}/{}] avg_train_loss={:.5f} avg_train_acc={:.5f}===='.format(epoch + 1, EPOCHS, avg_train_loss, avg_train_acc))
        ...
        if epoch == EPOCHS-1:#保存模型
            ...
            model_to_save = model.module if hasattr(model, 'module') else model
            # 如果使用预定义的名称保存，则可以使用`from_pretrained`加载
            output_model_file = os.path.join(output_dir, 'Pure_bert.bin')
            torch.save(model_to_save.state_dict(), output_model_file)
            print('Model Saved!')
    ...
    plt.savefig('./cache/bert_wwm_1.png')
    plt.close()
    # plt.show()
    ...
    plt.savefig('./cache/bert_wwm_2.png')
    plt.close()
    # plt.show()
    ...
    plt.xlabel("epoches")
    plt.ylabel("test acc")
    plt.savefig('./cache/bert_wwm_3.png')
    plt.close()
    # plt.show()
