...
def read_data(data_dir):
    data = pd.read_csv(data_dir, encoding_errors='ignore')
...
class InputDataSet():
    ...
    def __getitem__(self, item):
        text = str(self.data['content'][item])
        labels = self.data['label'][item]
        labels = torch.tensor(labels, dtype=torch.long)
        # one_hot_labels = one_hot(labels-1, num_classes=5)
        ## 手动构建
...
if __name__ == '__main__':
    ...
    train_dataset = InputDataSet(train, tokenizer=tokenizer, max_len=512)
    ...
    #print(batch['one_hot_labels'].shape)
